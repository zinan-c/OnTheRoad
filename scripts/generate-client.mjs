...
export function collectOperations(contract) {
  const operations = {};
  for (const [path, pathItem] of Object.entries(contract.paths ?? {})) {
    for (const [method, operation] of Object.entries(pathItem)) {
      ...
      const parameters = [...(pathItem.parameters ?? []), ...(operation.parameters ?? [])]
        ...
        .filter(Boolean)
        .map(({ name, in: location, required = false }) => ({
          name,
          in: location,
          required,
        }));
      ...
      operations[operation.operationId] = {
        ...
        successStatuses,
        contractTestOnly: operation["x-environment"] === "contract-test-only",
      };
    }
  }
  return operations;
...
export const generatedOperations = Object.freeze(${JSON.stringify(operations, null, 2)});

...
test("TC-A10-03 parses 5000 rows within fixed Spike time and RSS thresholds", () => {
  ...
  assert.equal(report.conclusion, "GO", JSON.stringify(report, null, 2));
  ...
  for (const format of Object.values(report.formats)) {
    ...
    assert.ok(format.p95Ms <= report.thresholds.maxP95MsPerFormat);
  }
  assert.ok(report.maxRssBytes <= report.thresholds.maxRssBytes);

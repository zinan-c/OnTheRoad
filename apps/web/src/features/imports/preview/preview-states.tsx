...
export function PreviewStates({ rows, onSkipErrors }: { readonly rows: readonly PreviewRow[]; readonly onSkipErrors: (ids: readonly string[]) => void }) {
  ...
  return <section aria-label="导入预览" className="previewStates">
    ...
    <table><thead><tr><th>行</th><th>状态</th><th>原始值</th><th>规范值</th><th>问题</th></tr></thead><tbody>{pageData.items.map((row) => <tr key={row.id}><td>{row.sourceRowKey}</td><td>{rowAction(row)}</td><td className="previewCell">{stringify(row.rawData)}</td><td className="previewCell">{stringify(row.normalizedData ?? {})}</td><td>{row.errors.map(({ field, message }) => <span key={`${field}-${message}`}>{field}: {message}</span>)}</td></tr>)}</tbody></table>

  ...
  return <section aria-label="图片画廊" className="gallery">
    <div className="galleryGrid">
      {attachments.length === 0 ? <p role="status">还没有图片</p> : attachments.map((attachment, index) => <GalleryCard
        ...
        actions={actions}
        open={() => setLightboxId(attachment.id)}
        move={(offset) => {
          const next = [...attachments];
          const destination = index + offset;
          if (destination < 0 || destination >= next.length) return;
          [next[index], next[destination]] = [next[destination]!, next[index]!];
          actions.reorder(next.map(({ id }) => id));
        }}
        canMoveUp={index > 0}
...
function GalleryCard({ attachment, actions, open, move, canMoveUp, canMoveDown }: { readonly attachment: GalleryAttachment; readonly actions: GalleryActions; readonly open: () => void; readonly move: (offset: number) => void; readonly canMoveUp: boolean; readonly canMoveDown: boolean }) {
  ...
  return <article className="galleryCard" data-status={attachment.status}>
    ...
    <label>说明<input
      ...
      onBlur={() => {
        if (caption !== attachment.caption) actions.updateCaption(attachment.id, caption);
      }}
    /></label>
    ...
    <div className="galleryActions">
      ...
      <button type="button" onClick={() => move(-1)} disabled={!canMoveUp} aria-label="前移图片">前移</button>
      <button type="button" onClick={() => move(1)} disabled={!canMoveDown} aria-label="后移图片">后移</button>
      <button type="button" onClick={() => actions.setCover(attachment.id)} aria-pressed={attachment.isCover}>设为封面</button>
      {confirmDelete ? <>
        <button type="button" onClick={() => actions.remove(attachment.id)}>确认删除</button>

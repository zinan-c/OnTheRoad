...
export function CostSummaryPanel({ summary, budget }: { readonly summary: CostSummary; readonly budget: string | null }) {
  ...
  return <section aria-label="Expense summary" className="costSummary">
    ...
    <div className="costDimensions">{["day", "destination", "category", "mode", "currency"].map((dimension) => <section key={dimension} aria-label={`${dimension} summary`}><h3>{dimension}</h3><ul>{dimensionRows(summary, dimension).map(([key, row]) => <li key={key}><span>{key}</span><span>{row.originalTotal} · settled {row.settledTotal}</span></li>)}</ul></section>)}</div>
  </section>;
}

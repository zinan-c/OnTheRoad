...
describe("TC-C08-01 route style contract", () => {
  test("keeps mode color, line and icon distinguishable", () => {
    ...
    expect(walk).toMatchObject({ label: "WALK", icon: "person-walking", isApproximate: false });
    expect(flight).toMatchObject({ label: "FLIGHT", icon: "plane", dasharray: [6, 4] });
  ...
  test("uses custom mode and labels approximate/unknown quality", () => {
    ...
    expect(style).toMatchObject({ label: "缆车接驳", color: "#123456", dasharray: [1, 2], qualityLabel: "Approximate route", isApproximate: true });
  });
});

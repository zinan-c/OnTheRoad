...
test("E2E-015 persists map pick, Marker drag and final manual WGS84 coordinates with successive ETags", async () => {
  ...
  fireEvent.change(screen.getByLabelText("Manual longitude"), { target: { value: "121.5100" } });
  fireEvent.change(screen.getByLabelText("Manual latitude"), { target: { value: "31.2200" } });
  fireEvent.click(screen.getByRole("button", { name: "Save manual coordinates" }));
  await screen.findByText(/Version 4.*121\.51, 31\.22.*manually adjusted/u);
...
test("E2E-015 surfaces a Location version conflict", async () => {
  ...
  fireEvent.click(screen.getByRole("button", { name: "Save map point" }));
  expect((await screen.findByRole("alert")).textContent).toContain("updated elsewhere");
});

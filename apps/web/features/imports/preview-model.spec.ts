...
import { buildPreviewCounts, filterPreviewRows, paginatePreviewRows, previewStageLabel, rowAction, skipErrorRows, type PreviewRow } from "../../src/features/imports/preview/preview-model";
...
describe("TC-E05-01 preview count invariant", () => {
  ...
  test("never claims staging rows are imported", () => {
    expect(rowAction(row("1", "error"))).toBe("需修正");
    expect(previewStageLabel()).toContain("尚未写入");
    expect(() => skipErrorRows([row("1", "error")], ["1"], false))
      .toThrow("PREVIEW_SKIP_CONFIRMATION_REQUIRED");
  });
});

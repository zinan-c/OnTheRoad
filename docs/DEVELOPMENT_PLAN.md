...
> 执行状态：M0–M3 已完成 Dev Track Gate；M4 为下一计划里程碑。当前状态与证据见 [文档状态索引](./README.md)。
...
- G08 已在实现前标记为 Deprecated；不招募 Beta cohort，不建设测试账号池，也不以固定真实用户样本数阻断 GA。
- 按 §13 的内部/staging → 受控 canary → 扩大范围 → 100% 门禁逐级放量。
- 只接受阻断缺陷、数据完整性、安全与性能回归修复；新功能回到 P1。
- 合成 fixture/探针可证明技术健康但不代表用户验证；是否开展后续用户研究由产品另行立项，不影响本计划的技术 Release Done。
...
| Sprint 1 | A05–A07、B01–B04、C01、C03、D01 | B02 完成后 B03 与 C03 由两名 BE 并行 |
...
| 稳定/灰度 | G01–G07 最终签署 | 自动化从功能 Sprint 起建设，不在最后两周从零开始 |
...
| G08 | **Deprecated**：Beta cohort、测试账号池和灰度样本运营（编号保留，不执行） | — | 0 | — | 已从 M1/M6 移出；见 [`deprecated/G08-beta-cohort.md`](./deprecated/G08-beta-cohort.md) |

当前有效任务合计约 **223 理想人日**，Spike 后仍以 220–230 区间校准。13 周目标（第 14 周缓冲）依赖 2 FE + 2 BE + 1 Worker/平台 + 1 QA 的并行能力；3–4 人精简团队按 18–22 周重新排程。G08 以 0 人日 Deprecated 行保留，仅用于历史追踪。
...
G08 已标记为 Deprecated，不再设 cohort Owner、测试账号池或真实样本数门槛。QA/平台负责人按技术健康、数据完整性、安全、容量、告警和回滚准备度归档每阶段证据。合成 fixture/探针可以验证技术行为，但不得被表述为真实用户验证；缺少测试用户本身不构成 No-Go。

| 阶段 | 最小观察/证据 | 继续条件 |
|---|---|---|
| 内部 | 24 小时；10 个独立 fixture 闭环、Import/PDF 各 10 个 | 可使用合成数据；0 数据丢失/越权/假成功；核心 5xx < 0.5% |
| 受控 canary（有生产流量时可从 5% 开始） | 至少 24 小时；自动探针、告警和回滚演练证据完整 | 不要求固定用户/Trip 数；系统原因 Job 失败 < 2%；最老可运行任务 < 5 分钟；Provider 429 < 1% |
| 扩大范围（有生产流量时可到 25%） | 至少 48 小时；持续健康窗口和人工复核记录 | 同上，且无新增 High 漏洞、无 ledger/路线/PDF 完整性差异 |
| 100% | 产品、QA、工程共同签署 | 所有 QG 与性能门禁归档 |

没有可分流的生产流量时，不伪造 5%/25% 样本：完成内部证据、人工签署、feature flag 和回滚准备后，可按发布审批直接进入受控生产观察。任一阶段出现以下情况立即暂停放量并关闭相关 feature flag：任何数据丢失/越权、恶意文件绕过、重复正式导入、PDF 状态成功但产物不可打开、核心 5xx > 2% 持续 10 分钟、最老可运行 Job > 10 分钟持续 15 分钟，或 Provider 429 > 5% 持续 15 分钟。数据/schema 不兼容时不自动回滚，进入事件响应；纯应用回归按下节回滚镜像。
